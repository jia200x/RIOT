...
{
    if (tx_data) {
        //event_post(EVENT_PRIO_MEDIUM, &event_tx_done);
    }
    tx_data = false;
...
void DSMEPlatform::processRxDone()
{
    ...
    if ((buf[0] & 0b11) == 1) {
       //event_post(EVENT_PRIO_MEDIUM, &event_data);
    }
...
void DSMEPlatform::initialize(bool pan_coord)
{
    ...
    this->dsmeAdaptionLayer.initialize(scanChannels, CONFIG_IEEE802154_DSME_SCAN_DURATION, scheduling);
...
bool DSMEPlatform::prepareSendingCopy(IDSMEMessage* msg, Delegate<void(bool)> txEndCallback)
{
    ...
    if ((mhr[0] & 0b11) == 1) {
       //event_post(EVENT_PRIO_MEDIUM, &event_tx_data);
       tx_data = true;
    }
