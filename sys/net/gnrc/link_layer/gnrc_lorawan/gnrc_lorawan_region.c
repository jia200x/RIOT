...
/*TODO: Reimplement */
static size_t _get_num_used_channels(void) {
    size_t count = 0;
    for(int i=0;i<16;i++) {
        if(lorawan_channels[i])
            count++;
    }
    return count;
}
...
/*TODO: Reimplement */
static uint32_t _get_nth_channel(size_t n) {
    int i=0;
    uint32_t channel = 0;
    while(n) {
        if(lorawan_channels[i]) {
            n--;
            channel = lorawan_channels[i];
            i++;
        }
    }
    return channel;
...
uint32_t gnrc_lorawan_pick_channel(gnrc_netif_t *netif) {
    ...
    return _get_nth_channel(random_number % _get_num_used_channels());
}
